...
foreign import customComponentImpl :: forall local. String -> Array String -> (Web.DOM.Element -> local -> Effect (Effect Unit)) -> Effect Unit
...
customComponent :: forall r rl. RowToList r rl => RL2Keys rl => String -> { | r } -> (forall lock payload. { | r } -> Array (Domable lock payload)) -> Effect Unit
customComponent s _ f = customComponentImpl s (rl2Keys (Proxy :: _ rl)) \e l ->
